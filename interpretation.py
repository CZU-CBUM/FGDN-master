...
def main(args):
    ...
    data1 = dd.io.load(os.path.join(args.vec_dir, 'Site20_correlation_matrix.h5'))
    data2 = dd.io.load(os.path.join(args.vec_dir, 'Site21_correlation_matrix.h5'))
    data3 = dd.io.load(os.path.join(args.vec_dir, 'Site25_correlation_matrix.h5'))
    ...
    idNYU = dd.io.load('./idx/Site20_sub_overlap.h5')
    idUM = dd.io.load('./idx/Site21_sub_overlap.h5')
    idUSM = dd.io.load('./idx/Site25_sub_overlap.h5')
    ...
    for i in range(5):
        ...
        elif args.site == 'Site21':
            test_loader = test_loader2
        elif args.site == 'Site25':
        ...
        plt.imshow(grad_b, cmap='hot')
        # plt.show()

    save_path = './interpretation_2class' if not args.MDD and not args.HC else (
        './interpretation' if args.MDD else './interpretation_hc')
    dd.io.save(os.path.join(save_path, args.method + '_' + args.site + '.h5'), {'grad': grad})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--method', type=str, default='single', help='[single, fed, mix]')
    parser.add_argument('--site', type=str, default='Site20', help='[Site20, Site21, Site25]')
    parser.add_argument('--MDD', action='store_true', help='if only test on MDD subjects')
    parser.add_argument('--HC', action='store_true', help='if only test on HC subjects')
    parser.add_argument('--vec_dir', type=str, default='./data/AAL_vector_overlap')
